...
ih, iw = item.shape[:2]
bg = tuple(int(c) for c in raw[0, 0])  # background colour from corner
...
final_img = np.full((final_size, final_size, 3), bg, dtype=np.uint8)
...
draw_rounded_rect(mask, (0, 0), (final_size-1, final_size-1), 24, 255)
...
print(f"Saved corrected SS Clamp -> {out}  ({final_size}x{final_size}px)")
