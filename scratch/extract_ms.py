...
img_path = r"C:\Users\MUHAMMADHU ANAS\OneDrive\Pictures\Screenshots\Screenshot 2026-07-13 003302.png"
im = cv2.imread(img_path)
h_img, w_img, _ = im.shape
print(f"Screenshot: {w_img}x{h_img}")

os.makedirs("public/ms_components", exist_ok=True)
...
# Fine-tuned column splits from divider scan
splits = [0, int(w_img/3), int(2*w_img/3), w_img]

products = [
    ("ms_boot_ring",  "MS Boot Ring"),
    ("ms_hook",       "MS Hook"),
    ("ms_base_plate", "MS Base Plate"),
]
...
for i, (name, label) in enumerate(products):
    ...
    cv2.imwrite(f"scratch/debug_ms_{name}.png", raw)
    ...
    if not contours:
        print(f"  No contours for {label}!"); continue

    largest = max(contours, key=cv2.contourArea)
    ox, oy, ow, oh = cv2.boundingRect(largest)
    print(f"  {label}: BBox x={ox}, y={oy}, w={ow}, h={oh}  (raw:{raw.shape[1]}x{raw.shape[0]})")
    ...
    out = f"public/ms_components/{name}.png"
